...
// offhours wraps JSX attributes across multiple lines, so the opening-tag scan
// must be multiline-aware (`s` flag) — a single-line regex would silently miss
// real backdrop/overlay <div onClick> handlers. Strictness still matches rotifolk:
// any <div>/<span> carrying an onClick should be a <button> or <a>/<Link>.
const CLICKABLE_NON_INTERACTIVE_RE = /<(div|span)\b[^>]*?\bonClick\s*=/gs
...
export function findClickableNonInteractiveElements(root = ROOT) {
  ...
  const files = listFiles(srcDir, (file) => {
    ...
    if (/[\\/]deskcloud[\\/]/.test(file)) return false // DeskCloud SDK integration (native components; report dialog uses radio inputs intentionally)
    return true
  })
  const findings = []
