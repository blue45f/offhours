/* eslint-disable react-hooks/set-state-in-effect, react-refresh/only-export-components -- 벤더링된 단일 파일 위젯(upstream chatdesk): 대화/스레드 동기화·SDK 부수 export 는 의도적. */
/**
 * ChatDesk (DeskCloud) — 단일 파일 벤더링 컴포넌트.
 * ──────────────────────────────────────────────────────────────────────────
 * chatdesk/apps-vendor/ChatWidget.tsx 를 offhours 로 벤더링한 버전입니다.
 * 워크스페이스 의존(@chatdesk/sdk·@chatdesk/shared) 0 — 상수·SDK·스타일을 인라인했습니다.
 *
 * offhours 적응(import 만 조정 — 동작 동일):
 *   원본은 `socket.io-client` 를 정적 import 했지만, offhours 에는 해당 의존성이 없고
 *   이 위젯은 VITE_CHATDESK_URL 이 설정될 때만 마운트되므로(기본 비활성),
 *   socket.io-client 를 **지연(dynamic) import** 로 바꿔 빌드가 해당 패키지를 강제하지
 *   않게 했습니다. 실시간 연결이 필요한 시점(대화 열기)에만 로드되며, 패키지가 없으면
 *   조용히 비연결 상태로 폴백합니다(REST 발송/히스토리는 그대로 동작).
 *
 * 사용:
 *   import { ChatWidget } from '@components/deskcloud/chat/ChatWidget'
 *   <ChatWidget publishableKey="pk_demo" endpoint={import.meta.env.VITE_CHATDESK_URL} memberId="alice" />
 *
 * 백엔드 계약:
...
/* ============================ socket.io 지연 로더 ============================ */
/* offhours 는 socket.io-client 를 정적 의존성으로 두지 않습니다. 위젯은 env 게이트로
   기본 비활성이고, 실시간이 필요할 때만 동적 import 합니다. 빌드 시 미설치라면 번들에
   external 로 남고, 런타임에 없으면 비연결 폴백(REST 만)으로 동작합니다.
   아래 ambient 선언으로 패키지 미설치 상태에서도 typecheck 가 통과합니다(최소 표면만). */

/** socket.io-client 의 최소 표면만 로컬 타입으로 선언. */
interface Socket {
  connected: boolean
  ...
  disconnect(): unknown
  timeout(ms: number): {
    emit(event: string, payload: unknown, ack: (err: Error | null, ack: never) => void): unknown
  }
}
type SocketIoFactory = (uri: string, opts: Record<string, unknown>) => Socket

let socketFactoryPromise: Promise<SocketIoFactory | null> | null = null

/** socket.io-client 를 1회 동적 로드. 미설치/실패 시 null(→ 비연결 폴백). */
function loadSocketIo(): Promise<SocketIoFactory | null> {
  if (!socketFactoryPromise) {
    // 변수 specifier 로 정적 모듈 해석을 피함(번들러는 런타임에 시도, 미설치면 catch).
    const specifier = 'socket.io-client'
    socketFactoryPromise = import(/* @vite-ignore */ specifier)
      .then(
        (mod: { io?: SocketIoFactory; default?: { io?: SocketIoFactory } }) =>
          mod.io ?? mod.default?.io ?? null
      )
      .catch(() => null)
  }
  return socketFactoryPromise
}
...
function createChatClient(options: ChatClientOptions): ChatClient {
  ...
  const ensureSocket = async (): Promise<Socket> => {
    ...
    if (socket) return socket
    if (!io) throw new ChatDeskError('실시간 연결 모듈(socket.io-client)을 불러올 수 없습니다.', 0)
  ...
  const api: ChatClient = {
    ...
    async connect() {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        const cleanup = (): void => {
          s.off('connect', ok as (...args: never[]) => void)
          s.off('connect_error', fail as (...args: never[]) => void)
        }
        s.once('connect', ok as (...args: never[]) => void)
        s.once('connect_error', fail as (...args: never[]) => void)
        s.connect()
      })
    },
    disconnect() {
