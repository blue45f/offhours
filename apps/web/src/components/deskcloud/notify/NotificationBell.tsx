/* eslint-disable react-hooks/refs -- 벤더링된 단일 파일 위젯(upstream notifydesk): 콜백 ref 패턴은 의도적. */
/**
 * NotifyDesk (DeskCloud) — 단일 파일 벤더링 컴포넌트 (의존성: react 만).
 * ──────────────────────────────────────────────────────────────────────────
 * notifydesk/packages/widget/src 의 <NotificationBell> 을 offhours 로 벤더링한 버전입니다.
 * 원본은 client.ts·icons.tsx·relative-time.ts·styles.ts·@notifydesk/shared 로 나뉘어 있었지만,
 * 다른 DeskCloud 위젯과 동일하게 **워크스페이스 의존 0 단일 파일**로 인라인했습니다(동작 동일).
 *
 * 사용:
 *   import { NotificationBell } from '@components/deskcloud/notify/NotificationBell'
 *   <NotificationBell recipientId="user_42" publishableKey="pk_demo" endpoint={import.meta.env.VITE_NOTIFYDESK_URL} />
 *
 * 백엔드 계약(publishable 키 · Bearer 헤더 + Origin):
...
/* ============================ 상대 시간(인라인) ============================ */
/* 외부 라이브러리 0(Intl.RelativeTimeFormat 만). "방금 전"·"3분 전"·"어제"… 그 이상은 절대 날짜. */
