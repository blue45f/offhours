 ...
 * ──────────────────────────────────────────────────────────────────────────
 * surveydesk/apps-vendor/FeedbackWidget.tsx 를 그대로 복사한 버전입니다.
 * npm publish 가 막힌 동안 형제 앱(offhours·resume·…)에 복붙해서 씁니다.
 * 워크스페이스 의존(@surveydesk/shared) 0 — 필요한 상수·검증 로직을 인라인했습니다.
 *
 * 사용:
...
function Rating({
  ...
  return (
    // Roving focus 는 focusable 한 <button role="radio"> 자식들이 담당하므로
    // 그룹 컨테이너 자체에는 tabindex 가 필요 없다(앱의 RatingInput 컨벤션과 동일).
...
export function FeedbackWidget(props: FeedbackWidgetProps): ReactElement | null {
  ...
  return (
    <div className="sd-root" style={rootStyle}>
      ...
      {open ? (
        // 배경 클릭으로 닫기(scrim). 키보드 동치는 document 레벨 Escape 핸들러가 제공하므로
        // 이 스크림에는 별도 role/키보드 핸들러가 필요 없다.
